...
def get_menu():
    menu = html.Div(
        [
            ...
            dcc.Link(
                "Price Performance",
                href="/business-scanner/price-performance",
                className="tab",
            ),
            dcc.Link(
...
# _______FUNCTIONS RELATED TO COMPUTATION_____

def extract_data(add):
    df = pd.read_excel(add, engine="openpyxl", header=4)
    df.drop("ITEM", axis=1, inplace=True)
    df.rename(columns={str(df.columns[0]): "id"}, inplace=True)

    # remove unnamed columns.
    df = df.loc[:, ~df.columns.str.contains('^Unnamed')]

    # turn all columns in low case
